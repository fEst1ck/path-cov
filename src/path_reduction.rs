...
impl<BlockID: Eq + Clone, FunID: Eq + Clone + Ord> PathReducer<BlockID, FunID> {
	pub fn reduce(&self, path: &[BlockID], cfg: FunID) -> Vec<BlockID> {
		...
		let (reduced_path, res) = re.parse_k(path, &self.res, self.k).expect("ill structured path");
		assert!(res.is_empty());
...
fn convert_cfgs(cfgs: BTreeMap<FunID, CFG<BlockID, FunID>>) -> BTreeMap<FunID, RegExp<BlockID, FunID>> {
	cfgs.into_iter()
		.map(|(fun_id, cfg)| {
